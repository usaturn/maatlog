...
@pytest.mark.browser
@pytest.mark.parametrize("theme", ["maatlog-base", None])
def test_button_appears_past_the_threshold_and_hides_again(
    built_site: BuiltSiteFactory,
    shared_browser: Browser,
    theme: str | None,
) -> None:
    result = built_site(theme=theme)
    context = shared_browser.new_context()
    try:
        page = context.new_page()
        _prepare(page, result.base_url)
...
@pytest.mark.browser
def test_threshold_boundary_is_600px(built_site: BuiltSiteFactory, shared_browser: Browser) -> None:
    result = built_site()
    context = shared_browser.new_context()
    try:
        page = context.new_page()
...
@pytest.mark.browser
def test_click_returns_to_the_top(built_site: BuiltSiteFactory, shared_browser: Browser) -> None:
    result = built_site()
    context = shared_browser.new_context()
    try:
        page = context.new_page()
...
@pytest.mark.browser
def test_click_jumps_without_animation_under_reduced_motion(
    built_site: BuiltSiteFactory,
    shared_browser: Browser,
) -> None:
    result = built_site()
    context = shared_browser.new_context(reduced_motion="reduce")
    try:
        page = context.new_page()
        _prepare(page, result.base_url)
        page.evaluate("() => window.scrollTo(0, 1200)")
...
@pytest.mark.browser
def test_keyboard_activation_moves_focus_to_the_top(built_site: BuiltSiteFactory, shared_browser: Browser) -> None:
    result = built_site()
    context = shared_browser.new_context()
    try:
        page = context.new_page()
...
@pytest.mark.browser
def test_mobile_button_clears_the_sidebar_toggle_and_hides_with_the_drawer(
    built_site: BuiltSiteFactory,
    shared_browser: Browser,
) -> None:
    result = built_site()
    ...
    try:
        page = context.new_page()
        _prepare(page, result.base_url, width=375, height=812)
        page.evaluate("() => window.scrollTo(0, 1200)")
        button = page.locator(".maatlog-back-to-top")
        expect(button).to_be_visible()
...
@pytest.mark.browser
def test_button_stays_absent_without_javascript(built_site: BuiltSiteFactory, shared_browser: Browser) -> None:
    result = built_site()
    context = shared_browser.new_context(java_script_enabled=False)
    try:
        page = context.new_page()
        page.goto(f"{result.base_url}{PAGE}", wait_until="load")
...
@pytest.mark.browser
def test_restored_scroll_position_shows_the_button_on_load(
