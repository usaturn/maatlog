...
def test_real_infinite_scroll_pages_two_and_three(
    ...
    site = real_sites.scroll(builder)
    ...
    base = site.base_url
    context = new_context(shared_browser, width=SCROLL_VIEWPORT["width"], height=SCROLL_VIEWPORT["height"])
    try:
        ...
        for expected_page in (2, 3):
            ...
            emitted = card_images_in(fetched_page_file(site.outdir, fetched_url))
...
def test_generated_basenames_are_uri_safe(
    real_sites: RealSites,
    tmp_path: Path,
    shared_browser: Browser,
    source_name: str,
    ...
    site = real_sites.named_source(source_name)
    samples: list[ResponseSample] = []
