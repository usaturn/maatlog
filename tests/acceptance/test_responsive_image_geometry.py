...
BOUNDARY_WIDTHS = [width for width in WIDTHS if width not in MAIN_WIDTHS]
...
def test_main_widths_geometry(
    ...
    app = build_image_site(make_sphinx, builder="html", theme=theme, responsive=True)
    with serve_directory(Path(str(app.outdir))) as base:
        ...
        try:
            ...
            for page_name in ("home", "archive", "post1", "profile", "showcase"):
                ...
                try:
                    page.goto(base + site_page("html", page_name), wait_until="load")
...
def test_boundary_widths_geometry(
    ...
    """Threshold ±1px widths keep exact slots on the relevant grids."""
    del tmp_path
...
def test_archive_counts_geometry(
    ...
    del tmp_path
    app = build_archive_site(
        make_sphinx, builder="html", theme="maatlog-default", responsive=True, post_count=post_count
    )
    with serve_directory(Path(str(app.outdir))) as base:
        context = browser.new_context(viewport={"width": width, "height": 900}, device_scale_factor=1)
        try:
            page = context.new_page()
            try:
                page.goto(base + site_page("html", "archive"), wait_until="load")
...
@pytest.mark.browser
@pytest.mark.parametrize("width", [575, 576, 577, 815, 816, 817, 1127, 1128, 1129, 2221, 2222, 2223, 2560])
def test_archive_thresholds_geometry(
    ...
    del tmp_path
    app = build_archive_site(make_sphinx, builder="html", theme="maatlog-default", responsive=True, post_count=9)
    with serve_directory(Path(str(app.outdir))) as base:
        context = browser.new_context(viewport={"width": width, "height": 900}, device_scale_factor=1)
        try:
            page = context.new_page()
...
def test_third_party_override_geometry(
    ...
    del tmp_path
    app = build_image_site(make_sphinx, builder="html", theme=THIRD_PARTY_THEME, responsive=True)
    with serve_directory(Path(str(app.outdir))) as base:
        context = browser.new_context(viewport={"width": width, "height": 900}, device_scale_factor=1)
        try:
...
@pytest.mark.browser
def test_dirhtml_geometry_smoke(make_sphinx: Callable[..., Sphinx], tmp_path: Path, browser: Browser) -> None:
    """dirhtml output keeps exact slots (builder independence spot check)."""
    del tmp_path
    app = build_image_site(make_sphinx, builder="dirhtml", theme="maatlog-default", responsive=True)
    with serve_directory(Path(str(app.outdir))) as base:
        ...
        try:
            for page_name in ("home", "post1"):
                page = context.new_page()
                try:
                    page.goto(base + site_page("dirhtml", page_name), wait_until="load")
                    assert _overflow(page) <= 1, page_name
                    _check_images(
                        page,
                        theme="maatlog-default",
...
@pytest.mark.browser
@pytest.mark.parametrize("theme", ["maatlog-default", "maatlog-base"])
def test_natural_dimensions_and_crop(
    make_sphinx: Callable[..., Sphinx], tmp_path: Path, browser: Browser, theme: str
) -> None:
    ...
    with serve_directory(Path(str(app.outdir))) as base:
        ...
        try:
            page = context.new_page()
            try:
                page.goto(base + site_page("html", "home"), wait_until="load")
                decode_all_images(page)
...
def test_display_and_top_wrapper_ratios(
    make_sphinx: Callable[..., Sphinx], tmp_path: Path, browser: Browser, theme: str, width: int
) -> None:
    ...
    with serve_directory(Path(str(app.outdir))) as base:
        ...
        try:
            ...
            try:
                home.goto(base + site_page("html", "home"), wait_until="load")
            ...
            try:
                post.goto(base + site_page("html", "post1"), wait_until="load")
